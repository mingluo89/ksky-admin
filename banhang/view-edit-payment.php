<?php
$sql1 = "SELECT * FROM orders_payment WHERE id ='" . $_GET['paymentid'] . "'";
$res1 = mysqli_query($connect, $sql1);
$count1 = mysqli_num_rows($res1);
if ($count1 > 0) {
    while ($row1 = mysqli_fetch_assoc($res1)) {
        $sql_nhap = "SELECT * FROM orders WHERE id ='" . $_GET['orderid'] . "'";
        $res_nhap = mysqli_query($connect, $sql_nhap);
        $count_nhap = mysqli_num_rows($res_nhap);
        if ($count_nhap > 0) {
            while ($row_nhap = mysqli_fetch_assoc($res_nhap)) {
                $remain = $row_nhap['total'];
                $sql_payment = "SELECT * FROM orders_payment WHERE orders_id ='" . $_GET['nhapid'] . "'";
...
?>
        ...
        <div class="row">
            <div class="col-12 col-md-6 offset-md-3">
                <div class="d-flex align-items-center justify-content-between py-3">
                    <a href="javascript:history.back()" class="btn ms-2"><span class="material-icons text-20 text-white">arrow_back_ios</span></a>
                    <div class="text-center">
                        <p class="text-white text-16">Đơn hàng #<?= $_GET['orderid']; ?></p>
                        <p class="fw-bold text-20 ">Sửa khoản thanh toán</p>
                    </div>
                    <div>
                        <a href="#" class="btn"></a>
                    </div>
            ...
            <div class="col-12 offset-md-2 col-md-8 offset-xl-3 col-xl-6">
                ...
                <div class="rounded-20 my-3 p-3">
                    <form action="do.php" method="post">
                        ...
                        <input type="hidden" name="order_id" value="<?= $_GET['orderid']; ?>" required>
                        <input type="hidden" name="payment_id" value="<?= $row1['id']; ?>" required>
                        ...
                        <div class="d-flex align-items-center mb-4">
                            ...
                            <input type="number" value="<?= $row1['amount']; ?>" class="form-control" id="amount" name="amount" min="0" max="<?= $remain; ?>" placeholder="Tối đa <?= number_format($remain, 0); ?> đ" required>
                        ...
                        <div class="d-flex mb-3">
                            ...
                            <div>
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" name="method" id="method1" value="Tiền mặt" required <?= ($row1['method'] == "Tiền mặt") ? "checked" : ""; ?>>
                                    <label class="form-check-label" for="method1">Tiền mặt</label>
                                </div>
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" name="method" id="method2" value="Chuyển khoản" required <?= ($row1['method'] == "Chuyển khoản") ? "checked" : ""; ?>>
                        ...
                        <div class="d-grid">
                            <button type="submit" class="btn btn-theme" id="submitBtn">Thêm</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <script>
            ...
            <?php
            if ($row1['method'] == "Tiền mặt") {
                echo "methodTienMat();";
            } else {
                echo "methodChuyenKhoan();";
            }
            ?>
        </script>
    <?php
