...
function update_nhap($connect, $nhap_id)
{
  // Calculate subtotal
  $sql1 = "SELECT SUM(total_before_vat) as total_before_vat FROM nhap_detail WHERE nhap_id = '$nhap_id'";
  $res1 = mysqli_query($connect, $sql1);
  while ($row1 = mysqli_fetch_array($res1)) {
    $total_before_vat = $row1['total_before_vat'];
  }

  // Find VAT
  $sql2 = "SELECT vat_rate FROM nhap WHERE id = '$nhap_id'";
  ...
  $sql3 = "UPDATE nhap SET total_before_vat = '$total_before_vat',vat_rate='$vat_rate',vat='$vat',total_after_vat='$total_after_vat' WHERE id = '$nhap_id'";
...
switch ($_POST['action']) {
  case 'add-nhap':
    ...
    // Step 2: Get next AUTO_INCREMENT value from nhap table
    $res_next_id = mysqli_query($connect, "
    SELECT AUTO_INCREMENT 
    FROM information_schema.TABLES 
    WHERE TABLE_SCHEMA = '$current_db' AND TABLE_NAME = 'nhap'");
    $row_next_id = mysqli_fetch_assoc($res_next_id);
    $next_id = $row_next_id['AUTO_INCREMENT'];
    $ksky_nhap_id = 'ksky' . str_pad($next_id, 6, '0', STR_PAD_LEFT);

    $accounting_nhap_id = mysqli_escape_string($connect, $_POST['accounting_nhap_id']);
    ...
    $sql = "INSERT INTO nhap (accounting_nhap_id,ksky_nhap_id,accounting_date,company_name,total_before_vat,vat_rate,vat,total_after_vat) VALUES ('$accounting_nhap_id','$ksky_nhap_id','$accounting_date','$company_name','$total_before_vat','$vat_rate','$vat','$total_after_vat')";
    echo $sql;
    if (mysqli_query($connect, $sql)) {
      $nhap_id = mysqli_insert_id($connect);
      header("Location: ./?view=detail&id=" . $nhap_id);
    } else {
  ...
  case 'edit-nhap':
    $id = mysqli_escape_string($connect, $_POST['nhap_id']);

    $accounting_nhap_id = mysqli_escape_string($connect, $_POST['accounting_nhap_id']);
    $accounting_date = mysqli_escape_string($connect, $_POST['accounting_date']);
    $company_name = mysqli_escape_string($connect, $_POST['company_name']);

    $sql_nhap = "SELECT * FROM nhap WHERE id = '" . $id . "'";
    $res_nhap = mysqli_query($connect, $sql_nhap);
    while ($row_nhap = mysqli_fetch_assoc($res_nhap)) {
      $total_before_vat = $row_nhap['total_before_vat'];
    }
    $vat_rate = mysqli_escape_string($connect, $_POST['vat_rate']);
    $vat = round($total_before_vat * $vat_rate / 100);
    $total_after_vat = $total_before_vat + $vat;
    ...
    $sql = "UPDATE nhap 
    SET accounting_nhap_id='$accounting_nhap_id', 
  ...
  case 'delete-nhap':
    $nhap_id = mysqli_escape_string($connect, $_POST['nhap_id']);

    $sql = "DELETE FROM nhap WHERE id = '" . $nhap_id . "'";
    if (mysqli_query($connect, $sql)) {
      $sql_detail = "DELETE FROM nhap_detail WHERE nhap_id = '" . $nhap_id . "'";
      if (mysqli_query($connect, $sql_detail)) {
        header("Location: ./");
      } else {
        echo "Error delete nhap_detail: " . mysqli_error($connect);
      }
    } else {
      echo "Error delete nhap: " . mysqli_error($connect);
    }
    mysqli_close($connect);
    break;
  ...
  case 'add-item':
    ...
    $sql_nhap = "SELECT * FROM nhap WHERE id = '$nhap_id'";
    $res_nhap = mysqli_query($connect, $sql_nhap);
    while ($row_nhap = mysqli_fetch_array($res_nhap)) {
      $ksky_nhap_id = $row_nhap['ksky_nhap_id'];
      $accounting_date = $row_nhap['accounting_date'];
      $accounting_nhap_id = $row_nhap['accounting_nhap_id'];
    }
    ...
    $is_it = mysqli_escape_string($connect, $_POST['is_it']);

    $sql = "INSERT INTO nhap_detail 
    (nhap_id,
    ksky_nhap_id,
    accounting_nhap_id,
    accounting_date,
    ...
    '$is_it')";
    if (mysqli_query($connect, $sql)) {
      if (update_nhap($connect, $nhap_id)) {
  ...
  case 'delete-item':
    ...
    $sql = "DELETE FROM nhap_detail WHERE id = '" . $item_id . "'";
